...
def get_last_loaded_date(tipo: str, ano: int, mes: int, slug: str) -> date | None:
    ...
    Tenta encontrar uma coluna que contenha 'data' no nome.
    Se não achar ou não conseguir converter, retorna None (faz carga completa do mês).
    """
    ...
    try:
        ...
        print(f"⚠️ Não consegui ler {path} para descobrir última data ({e}).")
        return None

    # tenta achar coluna de data
    col_candidates = [c for c in df.columns if "data" in c.lower()]
    if not col_candidates:
        print(f"⚠️ Não achei coluna de data em {path}; fazendo carga completa do mês se necessário.")
        return None

    col = col_candidates[0]
    try:
        dt_series = pd.to_datetime(df[col], dayfirst=True, errors="coerce").dropna()
    except Exception as e:
        print(f"⚠️ Erro ao converter datas da coluna {col} em {path}: {e}")
        return None
