...
for k,v in blow.estabelecimentos.items():
    ...
    try:
        if not os.path.exists('pesquisadesatisfacao/batch/'+str(ano)):
            os.makedirs('pesquisadesatisfacao/batch/'+str(ano))
    except OSError:
        print('Não foi possivel criar o diretório')
    file = 'pesquisadesatisfacao/batch/{}/{}.csv'.format(ano, slugEstabelecimento)
    print(file)
    try:
        os.remove(file)
